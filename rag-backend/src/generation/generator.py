...
class AnswerGenerator:
    ...
    TOP_CHUNKS_PER_SECTION = 3  # was 2 — allow more evidence per section
    ...
    def _section_preferences(sub_question: str) -> set[str]:
        ...
        if any(k in sq for k in ["result", "performance", "effective", "accuracy", "benchmark"]):
            return base | {"methodology", "conclusion"}
        if any(k in sq for k in ["what is", "define", "overview", "basic"]):
            return base | {"introduction", "related_work", "conclusion"}
    ...
    def _infer_section_from_content(self, text: str, labeled_section: str) -> str:
        ...
        # PRIORITY 1: Check for EXPLICIT section headers (most reliable)
        # These must be checked FIRST because they're definitive
        if "## results" in text_lower or "### results" in text_lower or "# results" in text_lower:
            return "Results"
        if "## methodology" in text_lower or "### methodology" in text_lower or "# methodology" in text_lower:
            return "Methodology"
        if "## introduction" in text_lower or "### introduction" in text_lower or "# introduction" in text_lower:
            return "Introduction"
        if "## discussion" in text_lower or "### discussion" in text_lower or "# discussion" in text_lower:
            return "Discussion"
        if "## conclusion" in text_lower or "### conclusion" in text_lower or "# conclusion" in text_lower:
            return "Conclusion"
        if "## dataset" in text_lower or "### dataset" in text_lower or "# dataset" in text_lower:
            return "Methodology"
        
        # PRIORITY 2: Check RESULTS first (strong indicators, low false positive rate)
        results_indicators = [
            "accuracy:", "accuracy =", "accuracy of", "f1 score", "precision:", "recall:",
            "auc-roc", "mean average precision", "achieved", "obtained", "performance:",
            "benchmark results", "evaluation results", "experimental results", "table ",
            "compared to baseline", "outperform", "sota", "state-of-the-art"
        ]
        if any(ind in text_lower for ind in results_indicators):
            return "Results"
        
        # PRIORITY 3: Check METHODOLOGY (specific implementations, not generic approaches)
        # MUST be specific to avoid false positives from introduction discussions of "approaches"
        methodology_specific = [
            "algorithm ", "pseudo-code", "step 1:", "step 2:", "procedure:",
            "our implementation", "implementation details", "hyperparameter",
            "training process", "network architecture", "layer", "batch size",
            "optimization", "loss function", "gradient descent"
        ]
        if any(ind in text_lower for ind in methodology_specific):
            return "Methodology"
        
        # PRIORITY 4: Check INTRODUCTION (motivation, problem statement)
        # Use STRONG indicators to avoid matching methodology discussions
        introduction_strong = [
            "problem statement", "research question:", "research gap",
            "motivation:", "why is", "importance of", "challenge is",
            "existing work", "literature review", "prior work", "related work",
            "this paper addresses", "this work focuses on"
        ]
        if any(ind in text_lower for ind in introduction_strong):
            return "Introduction"
        
        # PRIORITY 5: Fallback to labeled section if no strong indicators
        labeled = self._normalize_section(labeled_section)
        if labeled != "Unknown":
            return labeled
        
        # PRIORITY 6: If no labels, use weak cues
        # These are checked LAST to minimize false positives
        if any(word in text_lower for word in ["background", "literature", "state-of-art"]):
            return "Introduction"
        if any(word in text_lower for word in ["method", "approach", "framework"]):
            return "Methodology"
        if any(word in text_lower for word in ["discussion", "implication"]):
            return "Discussion"
    ...
    def _section_allowed_for_subquestion(self, section: str, subq: str) -> bool:
        """Section gating: Results and Discussion always relevant, then apply preferences."""
        ...
        # Methodology sub-questions accept methodology-related sections
    ...
    def generate_grouped_answer(
        ...
        for i, sub_q in enumerate(sub_questions, 1):
            ...
            if not filtered_chunks:
                ...
                if self.FALLBACK_TOP_K_PER_SUBQ > 0:
            ...
            if not filtered_chunks:
                ...
                output += "  This may indicate the retrieved papers do not directly address this aspect.\n\n"
                subq_data["mini_summary"] = "No relevant evidence found. Consider broadening the search query."
